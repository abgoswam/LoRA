...
def initialize():
    ...
    return args, enc, lm_net


def predict(contexts, args, enc, model):
    line_idx = 0
    ft_encoded_jsonls = []
    ...
    predictions = beam2(model, valid_loader, args)
    ...
    for i in range(len(predictions)):
        ...
        sample = _enc.decode(_pred_tokens).split('<|endoftext|>')[0].split('\n\n')[0].strip()
        predictions_sampled.append(sample)

    return predictions_sampled


if __name__ == "__main__":
    # initialize args, enc, model
    _args,  _enc, _model = initialize()
    ...
    preds = predict(_contexts, _args, _enc, _model)
